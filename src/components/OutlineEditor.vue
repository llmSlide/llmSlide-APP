...
const contextmenus = (el: HTMLElement): ContextmenuItem[] => {
  ...
  if (lv === 1) {
    ...
    return [
      {
        text: '上方新增同級項目（章）',//上方添加同级大纲（章）
        handler: () => addItem(id, 'prev', '新的一章'),
      },
      {
        text: '新增子項目（節）',//添加子级大纲（节）
    ...
    return [
      {
        text: '上方新增同級項目（節）',//上方添加同级大纲（节）
        handler: () => addItem(id, 'prev', '新的一節'),
      },
      {
        text: '新增子項目（項）',//添加子级大纲（项）
  ...
  return [
    {
      text: '上方新增同級項目（項）',//上方添加同级大纲（项）
      handler: () => addItem(id, 'prev', '新的一項'),
    },
    {
      text: '上方新增同級項目（項）',//下方添加同级大纲（项）
      handler: () => addItem(id, 'next', '新的一項'),
    },
    {
      text: '删除此項',
