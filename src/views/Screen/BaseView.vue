  ...
  <div class="base-view" :class="{ 'laser-pen': laserPen }">
    ...
    >
      <div class="content">
        ...
        <IconWrite class="tool-btn" v-tooltip="'畫筆工具'" @click="writingBoardToolVisible = true" />
        <IconMagic class="tool-btn" v-tooltip="'雷射筆'" :class="{ 'active': laserPen }" @click="laserPen = !laserPen" />
        <IconStopwatchStart class="tool-btn" v-tooltip="'計時器'" :class="{ 'active': timerlVisible }" @click="timerlVisible = !timerlVisible" />
        <IconListView class="tool-btn" v-tooltip="'演講者視角'" @click="changeViewMode('presenter')" />
        <IconOffScreenOne class="tool-btn" v-tooltip="'退出全螢幕'" v-if="fullscreenState" @click="manualExitFullscreen()" />
        <IconFullScreenOne class="tool-btn" v-tooltip="'進入全螢幕'" v-else @click="enterFullscreen()" />
        <IconPower class="tool-btn" v-tooltip="'結束放映'" @click="exitScreening()" />
      </div>
    </div>
...
const contextmenus = (): ContextmenuItem[] => {
  return [
    ...
    { divider: true },
    {
      text: autoPlayTimer.value ? '取消自動播放' : '自動播放',
