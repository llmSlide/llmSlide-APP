    ...
    <div class="container">
      ...
      <div class="send">
        <TextArea 
          ...
          :padding="6"
          :placeholder="`輸入註解（為${handleElementId ? '選中物件' : '目前投影片' }）`"
          :rows="2"
          @focus="replyNoteId = ''; activeNoteId = ''"
          @enter.prevent="createNote()"
        />
        <div class="footer">
          <IconDelete class="btn icon" v-tooltip="'清空本頁註解'" style="flex: 1" @click="clear()" />
          <Button type="primary" class="btn" style="flex: 12" @click="createNote()">新增註解</Button>
        </div>
      </div>
    </div>
  </MoveablePanel>
