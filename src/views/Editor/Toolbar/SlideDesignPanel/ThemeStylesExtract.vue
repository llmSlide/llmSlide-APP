<template>
  <div class="theme-styles-extract">
        ...
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.fontNames" :key="item">
            ...
            <div class="handler">
              ...
              <div class="config-btn" @click="updateTheme({ fontName: item }); selectedIndex.fontName = index">套用到主題</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.fontColors" :key="item">
            ...
            <div class="handler">
              ...
              <div class="config-btn" @click="updateTheme({ fontColor: item }); selectedIndex.fontColor = index">套用到主題</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.backgroundColors" :key="item">
            ...
            <div class="handler">
              ...
              <div class="config-btn" @click="updateTheme({ backgroundColor: item }); selectedIndex.backgroundColor = index">套用到主題</div>
            </div>
          </div>
        </div>
      </div>
      <div class="config-item" v-if="themeStyles.themeColors.length">
        <div class="label">主題色：<span class="tip">（點擊色塊以排除不需要的顏色）</span></div>
    ...
    <div class="btns">
      <Button class="btn" type="primary" @click="updateAllThemes()">將選取設定儲存為主題</Button>
    </div>
  </div>
</template>
...
const tabs: TabItem[] = [
  { key: 'single', label: '從當前頁擷取' },
  { key: 'all', label: '從全部投影片擷取' },
]
