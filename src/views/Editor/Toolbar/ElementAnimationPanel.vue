    ...
    <div class="element-animation" v-if="handleElement">
      ...
      >
        ...
        <Button class="element-animation-btn" @click="handleAnimationId = ''">
          <IconEffects style="margin-right: 5px;" /> 新增動畫
        </Button>
      </Popover>
    </div>

    <div class="tip" v-else><IconClick style="margin-right: 5px;" /> 選取投影片中的物件並新增動畫</div>
      ...
      <template #item="{ element }">
        <div class="sequence-item" :class="[element.type, { 'active': handleElement?.id === element.elId }]">
          <div class="sequence-content">
            ...
            <div class="handler">
              <IconPlayOne class="handler-btn" v-tooltip="'預覽'" @click="runAnimation(element.elId, element.effect, element.duration)" />
              <IconCloseSmall class="handler-btn" v-tooltip="'刪除'" @click="deleteAnimation(element.id)" />
            </div>
          </div>
          ...
          <div class="configs" v-if="handleElementAnimation[0]?.elId === element.elId">
            ...
            <div class="config-item">
              <Button style="width: 100%;" @click="openAnimationPool(element.id)">變更動畫</Button>
            </div>
          </div>
        </div>
      </template>
